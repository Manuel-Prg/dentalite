              ...
              {[
                ...
                <div key={s.num} className="flex items-center flex-1">
                  <div className="flex flex-col items-center flex-1">
                    ...
                    <span className={`text-xs mt-2 font-medium hidden sm:block transition-colors ${step >= s.num ? "text-sky-600" : "text-slate-500"
                      }`}>
                      {s.label}
                    </span>
                  </div>
                  {index < 3 && (
                    <div className={`h-1 flex-1 mx-2 rounded-full transition-all duration-500 ${step > s.num ? "bg-linear-to-r from-sky-500 to-blue-600" : "bg-slate-200"
                      }`} />
                  )}
                </div>
              ))}
